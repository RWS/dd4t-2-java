...
public class XPMComponentPresentationTag extends SimpleTagSupport {
    ...
    public void doTag() throws JspException, IOException {
        ...
        final String templateId = (String) pageContext.getAttribute(Constants.COMPONENT_TEMPLATE_ID, PageContext
                .REQUEST_SCOPE);
        final boolean isDynamic = (boolean) pageContext.getAttribute(Constants.DYNAMIC_COMPONENT_PRESENTATION,
                PageContext.REQUEST_SCOPE);

        XPMRenderer renderer = XPMRenderer.getInstance();

        final String out = renderer.componentPresentation(model.getTcmUri().toString(), model.getLastModified(),
                templateId, isDynamic);
