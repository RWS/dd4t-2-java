...
public abstract class BaseJSPViewHandler<T> implements IViewHandler<T> {
     ...
     * @throws Exception The dispatch can throw a variety of exceptions, such as
     *                   notably a FileNotFoundException when the request cannot be
     *                   dispatched
     */
    private String dispatch(Page page, T model, String URL, HttpServletRequest req, HttpServletResponse res) throws
            Exception {
        if (LOGGER.isDebugEnabled()) {
     ...
     */
    @Override
    public String handleView(Page page, T model, String ViewID, HttpServletRequest req, HttpServletResponse res)
            throws Exception {
